  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={add} className="flex items-center gap-2 px-4 py-2 bg-primary-pink text-white text-sm font-medium rounded-xl hover:bg-[var(--primary-pink-hover)] transition">
          <Plus size={16} /> Tambah
        </button>
      </div>
      {items.length === 0 && (
      ...
      {items.map((item, idx) => (
        <div key={item.id} id={`sertifikat-${item.id}`} className="border border-slate-200 rounded-2xl p-5 bg-slate-50 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-primary-pink">免許・資格 #{idx + 1}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2 flex flex-col gap-1.5">
              ...
              <input
                ...
                className="w-full border rounded-xl px-4 py-3 text-sm text-slate-900 bg-white border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-pink focus:border-transparent transition file:mr-3 file:border-0 file:bg-primary-pink-light file:text-primary-pink file:px-3 file:py-1.5 file:rounded-lg file:cursor-pointer"
              />
