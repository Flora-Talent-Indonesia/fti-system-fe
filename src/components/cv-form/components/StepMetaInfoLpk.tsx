...
const fileInputClass =
  "w-full border rounded-xl px-4 py-3 text-sm text-slate-900 bg-white border-slate-200 " +
  "focus:outline-none focus:ring-2 focus:ring-primary-pink focus:border-transparent transition " +
...
export default function StepMetaInfoLpk({
  meta,
  dokumen,
  info,
  onMetaChange,
  ...
  const setDokumen = (key: keyof DokumenPendukung, val: string) =>
    onDokumenChange({ ...dokumen, [key]: val });
  ...
  const handleGenerateKatakana = () => {
    ...
    setTimeout(() => {
      try {
        ...
        toast.success("Katakana berhasil di-generate! Periksa & koreksi jika perlu.", {
          duration: 4000,
        });
      } catch {
    ...
    <div className="space-y-8">
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField
            id="field-tanggal_pembuatan_cv"
            label="作成日 / Tanggal Masuk Pelatihan"
            value={meta.tanggal_pembuatan_cv}
            onChange={(v) => setMeta("tanggal_pembuatan_cv", v)}
            type="date"
            required
            error={errors.tanggal_pembuatan_cv}
          />
            ...
            <div className="flex flex-col xs:flex-row items-start gap-3">
              ...
              <div className="flex flex-col gap-2 flex-1 min-w-0">
                ...
                <label
                  ...
                  className="cursor-pointer px-4 py-2 text-sm bg-primary-pink text-white rounded-xl hover:bg-[var(--primary-pink-hover)] transition font-medium text-center w-full"
                >
                  Upload Foto
                </label>
      ...
      <section>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 pb-2 border-b border-slate-100 gap-4">
          ...
          <div className="w-full sm:w-56" id="field-no_peserta">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">
              NIM / No. Peserta <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={info.no_peserta}
              onChange={(e) => setInfo("no_peserta", e.target.value.toUpperCase())}
              placeholder="FTI-2026-001"
              className={`w-full border rounded-xl px-3 py-2 text-sm text-slate-900 bg-white focus:outline-none focus:ring-2 focus:border-transparent transition ${
                errors.no_peserta
                  ? "border-red-400 focus:ring-red-400"
                  : "border-slate-200 focus:ring-primary-pink"
              }`}
            />
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField
            id="field-yobisho"
            label="呼称 / Nama Panggilan"
            value={info.yobisho}
            onChange={(v) => setInfo("yobisho", v)}
            placeholder="Contoh: リナ"
            hint="Nama yang ingin dipanggil dalam bahasa Jepang"
            autoUppercase={false}
          />
          ...
          <div id="field-nama_katakana" className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2 items-stretch">
              <input
                ...
                placeholder="Contoh: リナ・ウランダリ"
                className="flex-1 border rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-pink focus:border-transparent transition bg-white border-slate-200"
              />
              <button
                type="button"
                ...
                className="inline-flex items-center gap-1.5 shrink-0 px-3 py-2 rounded-xl border border-primary-pink/40 bg-gradient-to-b from-primary-pink-light to-primary-pink-muted text-primary-pink text-xs font-semibold hover:border-primary-pink active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 shadow-sm"
              >
            ...
            <p className="text-xs text-slate-400">
              Cara baca nama dalam Katakana. Klik{" "}
              <span className="text-primary-pink font-medium">✨ Auto</span> untuk generate otomatis,
          ...
          />
          <FormSelect
            id="field-jenis_kelamin"
            label="性別 / Jenis Kelamin"
            value={info.jenis_kelamin}
            onChange={(v) => setInfo("jenis_kelamin", v)}
            options={JK_OPTIONS}
            required
            error={errors.jenis_kelamin}
          />
          <FormField
            id="field-kewarganegaraan"
            label="国籍 / Kewarganegaraan"
            value={info.kewarganegaraan}
            onChange={(v) => setInfo("kewarganegaraan", v)}
            placeholder="インドネシア"
            required
          ...
          <FormField
            ...
            onChange={(v) => setInfo("tanggal_lahir", v)}
            type="date"
            required
            error={errors.tanggal_lahir}
          />
          <FormSelect
            id="field-golongan_darah"
            label="血液型 / Golongan Darah"
            value={info.golongan_darah}
            onChange={(v) => setInfo("golongan_darah", v)}
            options={GOLDAR_OPTIONS}
          ...
          <FormSelect
            ...
            label="宗教 / Agama"
            value={info.agama}
            onChange={(v) => setInfo("agama", v)}
            options={AGAMA_OPTIONS}
            required
            error={errors.agama}
          />
          <FormSelect
            id="field-status_pernikahan"
            label="配偶者 / Status Pernikahan"
            value={info.status_pernikahan}
            onChange={(v) => setInfo("status_pernikahan", v)}
          ...
          <FormField
            ...
            label="郵便番号 / Kode Pos"
            value={info.kode_pos}
            onChange={(v) => setInfo("kode_pos", v.replace(/\D/g, "").slice(0, 5))}
            placeholder="12345"
            autoUppercase={false}
          />
          ...
          />
          <FormField
            id="field-alamat_lengkap"
            label="住所 / Alamat Lengkap"
            value={info.alamat_lengkap}
            onChange={(v) => setInfo("alamat_lengkap", v)}
            type="textarea"
            placeholder="RT/RW, Desa, Kecamatan, Kabupaten, Provinsi"
            required
