...
function startUrutForAngkatan(users: ListUserRow[], angkatPart: string): number {
  ...
  const inCohort = users
    ...
    .map((username) => {
      const match = username.match(/^FTI(\d{2})(\d{3})$/);
...
export default function AddAccountPage() {
  ...
  const fetchLatestNoPeserta = useCallback(async () => {
    const response = await FtiApiUser().getAllUser();
  ...
  const generatedAccounts = useMemo(() => {
    ...
    return nameLines.map((name, index) => {
      ...
      const nim = `FTI${angkatPart}${seq}`;
      const password = `FTI${angkatPart}`;
  ...
  const handleSaveAccounts = useCallback(async () => {
    ...
    try {
      ...
      const rows = names.map((name, index) => {
        ...
        return {
          name,
          user_name: `FTI${ang}${seq}`.toUpperCase(),
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="flex items-center">
                  ...
                  <input
                    ...
                    className="w-full min-w-0 px-4 py-2.5 border border-gray-300 border-l-0 border-r-0 focus:outline-none focus:ring-0 focus:border-primary-pink transition-colors font-mono"
                    placeholder="05"
                ...
                <p className="text-[10px] text-gray-500 mt-1.5 leading-relaxed">
                  ...
                  NIM berikutnya untuk angkatan ini: FTI<span className="font-bold text-[#be185d]">{angkatPart}</span>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border border-gray-300 focus:outline-none focus:ring-0 focus:border-primary-pink transition-colors text-sm leading-relaxed"
                ...
                <p className="text-[10px] text-gray-500 mt-1.5 leading-relaxed">
                  ...
                  Password default semua baris: <span className="font-mono">jukyu{angkatPart}</span>.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-transparent border border-gray-300 text-xs tracking-widest uppercase text-gray-600 hover:border-primary-pink hover:text-[#be185d] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2 bg-primary-pink border border-primary-pink text-xs tracking-widest uppercase text-white hover:bg-[var(--primary-pink-hover)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  ...
                  <tbody>
                    {generatedAccounts.map((acc) => (
                      <tr key={acc.id} className="bg-white border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-mono font-medium text-[#be185d]">{acc.nim}</td>
