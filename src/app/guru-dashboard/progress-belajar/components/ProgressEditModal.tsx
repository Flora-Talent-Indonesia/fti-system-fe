...
import { ProgressRow, AspectKey, aspectsConfig, NilaiSaveScope } from '../data';
    ...
    <div className="fixed inset-0 z-[120] bg-black/45 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-5xl max-h-[90vh] overflow-hidden bg-white border border-gray-200 shadow-2xl flex flex-col">
              ...
              <div className="bg-gray-50 p-4 border-b border-gray-200">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none bg-white border border-gray-300 text-gray-800 py-2.5 pl-4 pr-10 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#fc809f] focus:border-transparent rounded-lg shadow-sm transition-colors cursor-pointer uppercase tracking-wider"
                  >
                      ...
                      {currentConfig.columns.map((col) => (
                        <div key={col.key} className="bg-gray-50/50 p-4 border border-gray-200/80 rounded-xl flex flex-col justify-between">
                          ...
                          <select
                            ...
                            className="w-full border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#fc809f]/50 rounded-lg shadow-sm font-medium cursor-pointer"
                          >
                        ...
                        <table className="w-full text-sm text-left">
                          ...
                          <tbody className="divide-y divide-gray-200 bg-white">
                            {currentConfig.columns.slice(0, Math.ceil(currentConfig.columns.length / 2)).map((col) => (
                              <tr key={col.key} className="hover:bg-[#fc809f]/10/30 transition-colors group focus-within:bg-[#fc809f]/10/30">
                                ...
                                <td className="px-0 py-0 border-l border-gray-200">
                                  <input
                                    ...
                                    onChange={(e) => handleFieldChange(col.key, e.target.value, localAspect)}
                                    placeholder="-"
                                    className="w-full h-full border-0 bg-transparent py-2.5 px-4 focus:ring-inset focus:ring-2 focus:ring-[#fc809f] outline-none transition-shadow text-gray-800 font-medium"
                                  />
                                </td>
                              </tr>
                            ))}
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-white flex items-center justify-end gap-3 shrink-0">
          ...
          <button onClick={handleSave} disabled={isSaving} className="px-5 py-2.5 text-xs tracking-widest uppercase bg-[#fc809f] text-white hover:bg-[#fc809f] transition-colors disabled:opacity-75 disabled:cursor-not-allowed">
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        readOnly={readOnly}
        className={`mt-2 w-full border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#fc809f]/50 ${readOnly ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white text-gray-800'}`}
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={3}
        className="mt-2 w-full border border-gray-200 px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#fc809f]/50"
      />
    </label>
  );
}
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-full bg-gray-200 rounded-full h-2.5">
        <div
          className={`h-2.5 rounded-full transition-all duration-300 ${percentage >= 80 ? 'bg-green-500' : percentage >= 40 ? 'bg-[#fc809f]/100' : 'bg-yellow-500'}`}
